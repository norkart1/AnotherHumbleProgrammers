...
import { Gamepad2, Eye, Music, Trophy, CheckCircle, Loader2, RefreshCw } from 'lucide-react';
...
export default function BotStatusEditor() {
      ...
      <div className="px-5 py-4 border-b border-[#36393f] flex items-center justify-between">
        <div>
          ...
          <p className="text-[#72767d] text-xs mt-0.5">
            What the bot shows on its Discord profile
          </p>
        </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              {ACTIVITY_TYPES.map(({ value, label, icon: Icon, color }) => (
                <button
                  ...
                  className={`flex flex-col items-center gap-1.5 py-3 px-2 rounded-lg border transition-all text-xs font-medium ${
                    type === value
                      ? 'border-[#5865f2] bg-[#5865f2]/10 text-white'
                      : 'border-[#36393f] bg-[#1e2124] text-[#72767d] hover:border-[#4f545c] hover:text-[#b9bbbe]'
                  }`}
                >
          ...
          <button
            ...
            className={`w-full py-2.5 rounded-lg font-semibold text-sm transition-all flex items-center justify-center gap-2 ${
              status === 'saved'
                ...
                : 'bg-[#5865f2] hover:bg-[#4752c4] disabled:opacity-50 disabled:cursor-not-allowed text-white'
            }`}
          >
            ...
            {status === 'saving'
              ? 'Updating bot...'
              : status === 'saved'
              ? 'Updated! Bot status changed'
              : status === 'error'
              ? 'Failed — try again'
              : 'Update Bot Status'}
          </button>

          <p className="text-[#4f545c] text-xs text-center">
            Changes apply to the bot within ~10 seconds
          </p>
        </div>
      )}
    </div>
