...
import { Upload, CheckCircle, Loader2, AlertCircle, User, Image as ImageIcon, X } from 'lucide-react';
...
function ImageUploader({ field }: { field: ImageField }) {
  ...
  async function handleFile(file: File) {
    ...
    try {
      ...
      const res = await fetch('/api/bot-profile', {
        ...
        body: JSON.stringify({ type: field.key, dataUrl }),
      });
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          {isCircle ? (
            ...
            <ImageIcon size={14} className="text-[#eb459e]" />
          )}
          <span className="text-white text-sm font-semibold">{field.label}</span>
        </div>
        {field.note && (
          <span className="text-[#faa61a] text-[10px] font-medium bg-[#faa61a]/10 px-2 py-0.5 rounded-full">
            ⚠ Verified bots only
          </span>
        )}
      </div>
      ...
      >
        {preview ? (
          ...
          <div className="flex flex-col items-center gap-2 text-center px-4">
            <Upload size={22} className="text-[#4f545c]" />
            <p className="text-[#72767d] text-xs">
              Drag & drop or <span className="text-[#5865f2] font-medium">browse</span>
            </p>
            <p className="text-[#4f545c] text-[10px]">PNG, JPG, GIF — max {field.maxW}×{field.maxH}px</p>
          </div>
        )}
        <input
          ref={inputRef}
...
export default function BotProfileEditor() {
  return (
    <div className="bg-[#2c2f33] rounded-xl border border-[#36393f] overflow-hidden">
      <div className="px-5 py-4 border-b border-[#36393f]">
        ...
        <p className="text-[#72767d] text-xs mt-0.5">
          Change the avatar and banner shown on the bot&apos;s Discord profile
        </p>
      </div>
      <div className="p-5 grid grid-cols-1 sm:grid-cols-2 gap-4">
        {FIELDS.map((field) => (
          <ImageUploader key={field.key} field={field} />
        ))}
      </div>
      <div className="px-5 pb-4">
        <p className="text-[#4f545c] text-xs">
          Images are resized automatically. Avatar changes apply to all servers instantly.
        </p>
      </div>
    </div>
  );
